...
ros::Publisher left_info_pub[CAMERA_PAIR_NUM];
ros::Publisher right_info_pub[CAMERA_PAIR_NUM];
...
std::map<int, std::string> cam_position = {{0, "down"}, {1, "front"}, {2, "right"}, {3, "rear"}, {4, "left"}};
std::string frame_id;
...
std::string camera_params_left[CAMERA_PAIR_NUM];
std::string camera_params_right[CAMERA_PAIR_NUM];
...
int guidance_data_callback(int event, int data_len, char *content);
...
void image_handler(int data_len, char* content,  ros::Time timestamp){
    ...
    for (int i=0; i<CAMERA_PAIR_NUM; i++) {
            ...
            if ( data->m_greyscale_image_left[CAMERA_ID] ) {
                ...
                left_image_pub[CAMERA_ID].publish(left_8.toImageMsg());

                sensor_msgs::CameraInfo g_cam_info_left;
                g_cam_info_left.header.stamp = left_8.header.stamp;
                g_cam_info_left.header.frame_id = frame_id;

                try {
                    read_params_from_yaml_and_fill_cam_info_msg(camera_params_left[CAMERA_ID], g_cam_info_left);
                    left_info_pub[CAMERA_ID].publish(g_cam_info_left);
                } catch(...) {
                    printf("Camera ID: %d, YAML Loads failed, params cannot be loaded! \n", i);
            ...
            if ( data->m_greyscale_image_right[CAMERA_ID] ) {
                ...
                right_image_pub[CAMERA_ID].publish(right_8.toImageMsg());

                sensor_msgs::CameraInfo g_cam_info_right;
                g_cam_info_right.header.stamp = right_8.header.stamp;
                g_cam_info_right.header.frame_id = frame_id;

                try {
                    read_params_from_yaml_and_fill_cam_info_msg(camera_params_right[CAMERA_ID], g_cam_info_right);
                    right_info_pub[CAMERA_ID].publish(g_cam_info_right);
                } catch(...) {
                    printf("Camera ID: %d, YAML Loads failed, params cannot be loaded! \n", i);
                    // if yaml fails to read data, don't try to publish
                }
            ...
            if ( data->m_depth_image[CAMERA_ID] ) {
                ...
                depth_image_pub[CAMERA_ID].publish(depth_16.toImageMsg());
            }
        }
}
...
int main(int argc, char** argv)
{
    ...
    /* Setup the topic for each camera */
    /* Front Camera */
    for (int i=0; i<CAMERA_PAIR_NUM; i++) {
        /*Create image publisher*/
        std::string left_image_topic = "/guidance_" + cam_position[i] + "/left/image_raw";
        std::string right_image_topic = "/guidance_" + cam_position[i] + "/right/image_raw";
        std::string depth_image_topic = "/guidance_" + cam_position[i] + "/depth_image";
        left_image_pub[i] = my_node.advertise<sensor_msgs::Image>(left_image_topic, 1);
        right_image_pub[i] = my_node.advertise<sensor_msgs::Image>(right_image_topic, 1);
        depth_image_pub[i] = my_node.advertise<sensor_msgs::Image>(depth_image_topic, 1);

        /*Creat camera info publisher*/
        std::string left_info_topic = "/guidance_" + cam_position[i] + "/left/camera_info";
        std::string right_info_topic = "/guidance_" + cam_position[i] + "/right/camera_info";
        left_info_pub[i] = my_node.advertise<sensor_msgs::CameraInfo>(left_info_topic, 1);
        right_info_pub[i] = my_node.advertise<sensor_msgs::CameraInfo>(right_info_topic, 1);

        std::string left_param_file = "/guidanceNodeCalibration/" + cam_position[i] + "_cam_left_param_file";
        std::string right_param_file = "/guidanceNodeCalibration/" + cam_position[i] + "_cam_right_param_file";
        my_node.getParam(left_param_file, camera_params_left[i]);
        my_node.getParam(right_param_file, camera_params_right[i]);
    ...
    for (int i=0; i<CAMERA_PAIR_NUM; i++) {
        ...
        RETURN_IF_ERR(err_code);
        // err_code = select_depth_image(static_cast<e_vbus_index>(i));
        // RETURN_IF_ERR(err_code);
    }
    select_imu();
    select_ultrasonic();
    ...
    select_velocity();
    /* start data transfer */
    err_code = set_sdk_event_handler(guidance_data_callback);
    RETURN_IF_ERR(err_code);
    ...
    while (ros::ok())
    {
        ...
        for (int i=0; i<CAMERA_PAIR_NUM; i++) {
                ...
                if (i == 0){
                  ...
                  transform.setRotation( tf::Quaternion(-0.7071, 0, 0, 0.7071) );
                  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "base_link", frame_id));
                }
                else if (i == 2){
...
int guidance_data_callback(int event, int data_len, char *content) {
  ...
  switch (event) {
    case e_image:
      image_handler(data_len, content, timestamp);
      break;
    case e_imu:
      // imu_handler(data_len, content, timestamp);
      break;
    case e_velocity:
      // velocity_handler(data_len, content, timestamp);
      break;
    case e_obstacle_distance:
      // obstacle_handler(data_len, content, timestamp);
      break;
    case e_ultrasonic:
      // ultrasonic_handler(data_len, content, timestamp);
      break;
    // case e_motion:
    //   motion_handler(data_len, content, timestamp);
    //   break;
    default:
      break;
  }
